...
def maxCharTweet(event):
    if len(tweetContent.get("1.0", "end-1c")) == 140:
...
def checkContent():
    ...
    if 0 < len(content) <= 140 and 0 < len(name) <= 139:
        if "naam" in name:
            tkinter.messagebox.showwarning(title="Er ging iets fout", message="Vul een geldige naam in!")
        else:
            saveContent(content, name)
    else:
        tkinter.messagebox.showwarning(title="Er ging iets fout", message="De lengte van je tweet of je naam is niet correct!")
        return False
